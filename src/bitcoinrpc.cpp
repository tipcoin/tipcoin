...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "tipcoin server has now stopped running!";
}
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid tipcoin address");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid tipcoin address:")+s.name_);
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; tipcoin server stopping, restart to run with encrypted wallet";
}
...
//
// JSON-RPC protocol.  tipcoin speaks version 1.0 for maximum compatibility,
